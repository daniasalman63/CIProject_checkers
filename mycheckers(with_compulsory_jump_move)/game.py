...
class Game:
    ...
    def select(self, row, col):
        ...
        if piece != 0 and piece.color == self.turn:
            self.selected = piece
            self.valid_moves, jump = self.board.get_valid_moves(piece)
            return True
    ...
    def _move(self, row, col):
        ...
        piece = self.board.get_piece(row, col)
        priority = self.board.priority_moves(self.turn)
        if len(priority)>0:
            if self.selected and piece == 0 and (row, col) in self.valid_moves and (row,col) in priority:
